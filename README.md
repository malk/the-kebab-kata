...
Your interactions with your team mates and managers are as much of a burdan to your code as you are.
...
* At least 2 pairs of devs (Each group needs at least 2 devs, you need at least two groups)
 ...
 * You should always behave like it is too much and ask if they cannot do it in less
  * yes even as you decided on an arbitrary budget of time
* At the end of each iteration you each team has 45 seconds to do a “client demo”
 * During the client demos other teams can cheat and keep developing
 * the team does not show code in the client demo just tests (automatic or manual) of the code working
 ...
 * encourage teams to be honest with the reviewers
  * and reviewers to try to give positive suggestions on improvements
* after for iterations (each with its own code round of demos and code reviews): do a final debrief
...
* you will be generous and give them ample amount to do such a *simple* task : 10 minutes.
...
* Devalue their work
 * Treat the task like is trivial and 10 minutes like being more than double the time they need to do this
* Interrupt them to ask fr opinions and lower estimates
 * anyone that accepts to do it less has a reminder at the end of their estimate that they shoud be finished
* Insist that you think they should nto invest too much of their time into quality and tests
...
* ALSO by the way there is just You want `Kebab` to have another method `isPesceterian`
 * Everythign that `isVegeterian`is also `isPesceterian` *plus* fishs & shrimps
  * it is the first tiem you talk about shrimps
  * deny it being the first
  * tell them that you also want to submit a bug report regarding they not handling shrimp up until now
...
* go around team per team asking each team if they would not produce more if they split the team into several teams of oneperson each working together by splitting tasks

### Iteration 3: “A single urgent user story for this spring!” (15 minutes)
#### Tell the devs:
* You want to submit a bur report: they do not handle sauces
 * tell them is obvious you have sauces, of course you do
* but anyway this is not the main issue witht the sprint
* you have a really urgent user story!
...
* you are all *fired*!
* you are a new versons of yourselves, unrelated to the previous ones
 ...
 * each team shoudl give a report of what they would want to change in that legacy codebase they just audited


### Iteration 4: “Cleaning that legacy codebase!” (20 minutes)
#### Tell the devs:
* If the code does nto work, first make it work
* If the code does nto have tests you should add then
* Then you can refactor as you see fit
* I have hired an *Architect* to help you with an idea oh how to do that refactoring
 ...
 * They will take another, iner, `Kebab`in their cosntructor & delegate to it
 * so a `Beef` `Kebab` is not Vegeterian its `isVegeterian` calls the `isVegeterian` of its iner `Kebab`
...
* You have used at least 10 techniques to push them into creatign shitty code (Note: those where my Tips)
